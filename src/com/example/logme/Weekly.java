...
public class Weekly extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		String endDate = db.converter(pastDate) + "/" + db.converter(pastMonth) + "/" + year;
		
		Log.d("Weekly", "CurrentDate: " + currentDate);
		Log.d("Weekly", "endDate: " + endDate);

		List<String> lTitle = db.getDistinctAppNamesFromWeek(currentDate, endDate);
		List<Double> lTime = new ArrayList<Double>();
		for (String cn : lTitle) {
	    	
	    	double totalTime = db.getAppTimeFromWeeks(cn, currentDate, endDate);
	    	
	    	lTime.add(totalTime);
	    	
		}

		LinearLayout linearLayout = (LinearLayout) findViewById(R.id.weekly);
